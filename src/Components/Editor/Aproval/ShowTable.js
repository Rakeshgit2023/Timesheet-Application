...
const ShowTable=({onDelete, showDel, handleData, status, taskinfo, index})=>{
    ...
    const handelGetTaskData=(employeeId)=>{
     ...
     useEffect(()=>{
        //handelGetTaskData(JSON.parse(Cookies.get('approvalTask')).employeeId);
       },[])
       const handelSun=(e)=>{
    ...
    return(
        <div className="flex justify-start items-start mb-3">
                ...
                <div className="w-1/12 flex items-center justify-center mt-4">
                            {showDel && (status==='draft' || status==='') ? <RiDeleteBin6Line className="cursor-pointer font-medium text-lg lg:text-3xl text-red-500" onClick={handelDelete}/> : <RiDeleteBin6Line className="cursor-pointer font-medium text-lg lg:text-2xl text-zinc-200"/>}
                            </div>
            </div>
    )
}
