...
import TeamShowTable from "./TeamShowTable";
        ...
        <div className="flex flex-col mt-8 pl-6 pr-3 lg:px-12 py-0" style={{ height: '700px' }}>
                        ...
                        {[...Array(7)].map((_, index) => {
                            ...
                            return (
                                <div className={`flex flex-col justify-center items-center w-1/4 ${index % 2 === 0 ? 'bg-slate-400' : 'bg-slate-300'}`} key={index}>
                                    <span className="font-normal text-xs text-base">{format(currentDate, 'dd')}</span>
                                    <span className="font-normal text-xs text-base">{format(currentDate, 'EEE')}</span>
                                </div>
                            );
                        })}
                    </div>
            ...
            <div className="flex flex-col lg:flex-row justify-between lg:items-center w-full mt-5">
                ...
                <div className='flex  gap-3 lg:gap-10 ml-44 mt-10 lg:mt-0 lg:ml-0'>
                    {submittedTask.status === 'submit' ? <button className={true ? 'relative inline-flex px-3 py-1 lg:px-8 lg:py-3 text-sm lg:text-xl bg-gray-200 font-semibold text-gray-400 traking-widset rounded-full border-solid border-2 border-gray-400 ' : 'relative inline-flex px-3 py-1 lg:px-8 lg:py-3 text-sm lg:text-xl font-semibold traking-widset bg-slate-400  hover:bg-slate-600 hover:text-white rounded-full bg-gray-300'} onClick={() => handleEdit('approved')} >Approved</button> : null}
                    {submittedTask.status === 'submit' || submittedTask.status === 'approved' ? <button className='relative inline-flex px-3 py-1 lg:px-8 lg:py-3 text-sm lg:text-xl font-semibold traking-widset bg-slate-400  hover:bg-slate-600 hover:text-white rounded-full bg-gray-300 mr-10 ' onClick={() => handleEdit('rejected')}>Reject</button> : null}
                </div>
            </div>
        </div>
    )
